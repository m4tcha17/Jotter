...
- **Module shape**: a new local Expo module at `modules/jotter-camera/` (`expo-module create` convention — own `android/` Kotlin source, own `index.ts` TS API, own `expo-module.config.json`), registered as a config plugin in `app.json`'s `plugins` array alongside the existing `expo-camera` entry. `expo-camera` stays installed, but only for its permission hook — no capture code from it is used by the new module. (Path corrected from the originally-approved `modules/native/jotter-camera/` — see the plan's "Plan deviation from the committed spec" section for why.)
